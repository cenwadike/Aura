...
import { FacilitatorConfig, paymentMiddleware, RoutesConfig } from "x402-express";
...
const FACILITATOR_URL_HTTPS = FACILITATOR_URL as `https://${string}`;
...
// ==================== Revenue Tracking/Sharing (AVAX only) ====================
 ...
 */
async function sendSignedTransfer(
    to: string,
    value: bigint,
    nonce: number
): Promise<string> {
...
const createDynamicPaymentMiddleware = (): express.RequestHandler => {
    ...
    const facilitatorConfig: FacilitatorConfig = { url: FACILITATOR_URL_HTTPS };

    return async (req: Request, res: Response, next: NextFunction) => {
        const routeKey = `${req.method} ${req.path}` as keyof RoutesConfig;
        const routeConfig = routesConfig[routeKey];

        const getPrice = (cfg: any): string | null => {
            if (!cfg) return null;
        ...
        // Free routes
        if (price === "$0.00") {
            return paymentMiddleware(PLATFORM_TREASURY, routesConfig, facilitatorConfig)(req, res, next);
        }
...
app.use(createDynamicPaymentMiddleware() as express.RequestHandler);
